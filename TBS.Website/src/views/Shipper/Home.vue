          ...
          <table class="table table-bordered table-hover text-center">
            ...
            <tbody>
              <tr v-for="post in posts" :key="post.id">
                ...
                <td>
                  <router-link v-if="parsePostStatus(post.postStatus) == 'Open' || parsePostStatus(post.postStatus) == 'Pending Delivery' || parsePostStatus(post.postStatus) == 'Pending Delivery Approval'" class="btn btn-main bg-blue fade-on-hover text-uppercase text-white" :to="{ name: 'shipperManageBids', params: { id: post.id } }">Manage Bids</router-link>
                  <router-link v-else class="btn btn-main bg-blue fade-on-hover text-uppercase text-white" :to="{ name: 'shipperManageBids', params: { id: post.id } }">Archived Bids</router-link>
                </td>
                <td>
        ...
        <div class="row">
          <table class="table table-bordered table-hover text-center">
            <thead>
              ...
              <th style="width: 25%">Bid Status</th>
              <th style="width: 25%">Management</th>
            </thead>
            <tbody>
...
export default {
  ...
  methods: {      
    ...
    fetchPosts() {
      this.$store.dispatch('posts/getMyPosts', { type: 'shipper', currentPage: this.postPage, pageSize: 5 })
        .then((response) => {
